...
io.on('connection', function (socket) {
  ...
  socket.on('frndRq-accepted', function (data) {
    ...
    // Notifico
    if (clients[data[0]]) {
      io.sockets.connected[clients[data[0]]["id"]].emit('refresh-frnd&frndreq-bar', [registeredClients[data[0]]["friendReq"], registeredClients[data[0]]["friends"]]);
    }

    if (clients[data[1]]) {
      io.sockets.connected[clients[data[1]]["id"]].emit('refresh-frnd&frndreq-bar', [registeredClients[data[1]]["friendReq"], registeredClients[data[1]]["friends"]]);
    }
  });
  ...
  socket.on('registration-login', function (reqName) {
    ...
    if (checkClients(reqName[0], clients)) {
      ...
      console.log("[INFO] name already exist: " + reqName[0])
